...
export default function BudgetForm({ budget, onClose }: BudgetFormProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center p-4 z-50">
      <div className="bg-(--color-surface) rounded border border-(--color-border) max-w-md w-full p-8 card">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label htmlFor="name" className="block text-sm font-semibold text-(--color-text) mb-2">
              Budget Name
            </label>
            <input
              type="text"
          ...
          <div>
            <label htmlFor="amount" className="block text-sm font-semibold text-(--color-text) mb-2">
              Allocated Amount ($)
            </label>
            <input
              type="number"
              ...
              step="0.01"
              min="0"
              className="w-full px-4 py-2 border-2 border-(--color-border) rounded focus:outline-none focus:border-(--color-accent) disabled:bg-gray-100 disabled:cursor-not-allowed transition-all duration-200 font-mono"
              placeholder="0.00"
            />
          </div>

          <div>
            <label htmlFor="category" className="block text-sm font-semibold text-(--color-text) mb-2">
              Category
            </label>
            <select
              id="category"
          ...
          <div>
            <label htmlFor="budgetDate" className="block text-sm font-semibold text-(--color-text) mb-2">
              Budget Date
            </label>
            <input
              type="date"
          ...
          <div className="flex gap-4 pt-2">
            <button
              type="submit"
              disabled={isLoading}
              className="flex-1 bg-(--color-accent) disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold py-2 px-4 rounded transition-colors duration-200 hover:bg-(--color-accent-hover) focus:outline-none focus:ring-2 focus:ring-(--color-accent) focus:ring-offset-2"
            >
            ...
            <button
              ...
              onClick={onClose}
              disabled={isLoading}
              className="flex-1 border-2 border-(--color-border) text-(--color-text) disabled:bg-gray-50 disabled:cursor-not-allowed font-semibold py-2 px-4 rounded transition-colors duration-200 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-(--color-accent) focus:ring-offset-2"
            >
              Cancel
            </button>
          </div>
